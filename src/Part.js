...
class Part extends Component {
    ...
    return (
    <fieldset key = {this.props.featureHash}
    className = "feature" >
        ...
        { this.props.features.map((name, index) =>
            ...
            <div 
                    key = {slugify(JSON.stringify(name))}
                    ...
                    onChange = {
                        () => this.props.select(this.props.featureName, name)
                    } /> 
                <label htmlFor = {slugify(JSON.stringify(name))}
                        className = "feature__label" > 
                        { name.name}
                        ({ this.props.currency.format(name.cost)}) 
                 </label> 
             </div>
            )
        } 
